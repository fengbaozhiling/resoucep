...
;(function($){
	$.fn.floatFollow = function( options ) {
		...
		var htmlwrap=$("<div style='height:"+_self.height()+"px;width:"+_self.width()+"px; position:relative;'><div style='height:"+_self.height()+"px;width:"+_self.width()+"px; position:relative;'></div></div>"),
			closebar=$("<i class='closflow' style=' position:absolute;right:5px;top:5px;z-index:10;'>close</i>");
		_self.wrap(htmlwrap);
		closebar.insertBefore(_self);
		closebar.toggle(function(){
			_self.hide();
		},function(){
			_self.show();
		})
		...
		if(Agentie > 0){
			if (starheightt=false) {
				_self.css(settings).css('position', 'absolute');
		...
		settings.callback;
		return _self.parent(); //返回this，使方法可链。 
	}
